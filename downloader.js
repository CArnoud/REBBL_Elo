...
for (let i = 0; i < seasonNames.length; i++) {
    request.get(rebblNetHost + '/division/' + leagueNames.REL + '/' + seasonNames[i], (error, response) => {
        ...
        fs.mkdir('files/' + seasonNames[i], { recursive: true }, (err) => {
            if (!err || updateAll) {
                for (let j = 0; j < divisionNames.length; j++) {
                    request.get(rebblNetHost + '/division/' + leagueNames.REL + '/' + seasonNames[i] + '/' + divisionNames[j], (error2, response2) => {
                        const games = JSON.parse(response2.body);
                        const season = parseSeason(games);            
            
                        const fileName = 'files/' + seasonNames[i] + '/' + leagueNames.REL + '.' + divisionNames[j] + '.json';    
